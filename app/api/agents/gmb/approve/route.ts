...
import { verifyToken } from '@/src/lib/agents/gmb-approval-gate'
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Approved — BLOOM ENGINE</title>
</head>
<body style="margin:0;padding:0;background:#0D0D0D;">
  <div style="background:#0D0D0D;color:#EDEDED;font-family:'IBM Plex Mono',monospace;max-width:600px;margin:0 auto;padding:60px 40px;text-align:center;">
    <div style="font-size:10px;color:#D4AF6A;letter-spacing:0.2em;text-transform:uppercase;margin-bottom:32px;">BLOOM ENGINE</div>
    <div style="font-size:48px;margin-bottom:20px;">&#x2705;</div>
    <div style="font-size:18px;color:#D4AF6A;font-weight:700;margin-bottom:16px;letter-spacing:0.05em;">Post Approved!</div>
    <div style="font-size:12px;color:#94A3B8;line-height:1.8;">
      Your Google Business post has been approved.<br>
      It will go live at <strong style="color:#EDEDED;">10:00 AM PT</strong> today.
    </div>
    <div style="margin-top:32px;padding:16px;background:#161616;border-left:2px solid #D4AF6A;text-align:left;">
      <div style="font-size:10px;color:#94A3B8;letter-spacing:0.08em;">You don't need to do anything else — we'll handle the rest.</div>
    </div>
    <div style="font-size:9px;color:#3F3F3F;letter-spacing:0.1em;margin-top:60px;">POWERED BY BLOOM ENGINE &times; ANCHOR</div>
  </div>
</body>
...
// ---------------------------------------------------------------------------
// GET /api/agents/gmb/approve?token=...
// ---------------------------------------------------------------------------
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (!payload) {
    return errorPage(
      'This approval link has expired. Please check tomorrow\'s email for a new post.'
    )
  }
  ...
  try {
    // 2. Load client config (validates client exists)
    await loadClientConfig(clientId)

    // 3. Update gmb_post_log: set status = 'approved'
    ...
    fetch(`${baseUrl}/api/agents/gmb/post-approved`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ postId, clientId }),
    }).catch((err) => {
      console.error('[gmb/approve] Failed to trigger post-approved:', err)
    })
